...
class YHash {
  ...
  public static fromHex(hexString): YHash {
...
function nodeToJSON(node: TreeNode): TreeNodeJSON {
  return {
    ...
    left: node.left ? nodeToJSON(node.left) ,
    right: node.right ? nodeToJSON(node.right) ,
  };
}
...
class StorageGatewayClient {
  constructor(private readonly storageGatewayUrl) {}

  public getStorageGatewayUrl() {
...
export class StorageClient {
  ...
  private async getCertificate(hash): Promise<Uint8Array> {
  ...
  public async getDirectURL(hash): Promise<string> {
    if (!hash) {
  ...
  private async parallelUpload(
    ...
    const uploadSingleChunk = async (index): Promise<void> => {
